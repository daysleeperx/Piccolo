...
export const eventsToNote = ([_ticks, msg] : Midi.Event, [ticks, _msg] : Midi.Event) : MusicGenerator.Note => {
    return [(msg as Midi.NoteOn).note, ticks];
}
...
export function extractSequenceFromTrack(track: Midi.Track, tempo: Midi.Tempo, division: Midi.TimeDivision): MusicGenerator.Sequence {
    return {
        notes: track
            .filter(([_, msg]) => isNoteMessage(msg))
